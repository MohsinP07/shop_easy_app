...
userRouter.post("/api/increase-quantity", auth, async (req, res) => {
    try {
        const { id } = req.body;
        const product = await Product.findById(id);
        let user = await User.findById(req.user);
...
userRouter.delete("/api/remove-from-wishlist/:id", auth, async (req, res) => {
    try {
        ...
        for (let i = 0; i < user.wishlist.length; i++) {
            if (user.wishlist[i].product._id.equals(product._id)) {
                if (user.wishlist[i].quantity == 1) {
                    user.wishlist.splice(i, 1);
                } else {
                    user.wishlist[i].quantity -= 1;
                }
            }
        }
        user = await user.save();
...
userRouter.post("/api/order", auth, async (req, res) => {
    try {
        const { cart, totalPrice, address, sellerId } = req.body;
        let products = [];
